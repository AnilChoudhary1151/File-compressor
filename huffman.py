...
class Huffmancode:
    ...
    def compression(self):
        ...
        os.makedirs(filename+'_compressed', exist_ok=True)
        output_path = filename+'_compressed' + '//data.bin'
        metadata_path=filename+'_compressed' + '/.metadata.txt'
        
        with open (self.path,'r+') as file , open(output_path,'wb') as output ,open(metadata_path,'w+') as mdata:
            text = file.read()
            text = text.rstrip()
            ...
            mdata.write(str(self.__reversecode))
    ...
    def __Decompress_Text(self,text,rcode):    
        decoded_text = ''
        current_bits = ''
        for bit in text:
            current_bits += bit
            if current_bits in rcode:
                character = rcode[current_bits]
    ...
    def decompress(self):
        input_path = os.path.join(self.path, 'data.bin')
        meta_path = os.path.join(self.path, '.metadata.txt')
        
        filename,file_extension = os.path.splitext(input_path)
        output_path = self.path + '_decompressed' + '.txt'
        
        with open(input_path,'rb') as file ,open(meta_path,'r+') as mdeta, open(output_path,'w') as output:
            bit_string = ''
            byte = file.read(1)
            while byte:
                byte = ord(byte)      
            ...
            rcode=eval(mdeta.read().rstrip())
            decompressed_text = self.__Decompress_Text(actual_text,rcode)
